...
# ... (code for eye_status, init, process_and_encode, isBlinking functions)
def init():
...
class IntegratedFaceRecognitionLiveness():
    def __init__(self):
        self.model, self.face_detector, self.open_eyes_detector, self.left_eye_detector, self.right_eye_detector, self.video_capture, self.images = init()
        self.data = process_and_encode(self.images)
        self.eyes_detected = defaultdict(str)
            ...
            if len(open_eyes_glasses) == 2:
                ...
                for (ex, ey, ew, eh) in left_eye:
                    ...
                    if pred == 'closed':
                        ...
                        color = (0, 0, 255)
                    cv2.rectangle(left_face, (ex, ey), (ex + ew, ey + eh), color, 2)
                self.eyes_detected[name] += eye_status

            if isBlinking(self.eyes_detected[name], 3):
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                y = y - 15 if y - 15 > 15 else y + 15
                cv2.putText(frame, name, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)

        self.face_recognition.face_locations, self.face_recognition.face_encodings, self.face_recognition.face_names = [], [], []

        return frame

    def run_integration(self):
        while True:
            frame = self.detect_and_display()
            cv2.imshow("Integrated Face Recognition and Liveness Detection", frame)
...
if __name__ == "__main__":
    integrated_system = IntegratedFaceRecognitionLiveness()
    integrated_system.run_integration()
